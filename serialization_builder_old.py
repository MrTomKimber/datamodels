...
def generate_elemtree_header(serialization_label, serialization_iri, namespaces, ontology, mappings, translation_mappings):
    ...
    # Where a StaticPropertyMapping exists, it will likely target some property (e.g RDFS:type) that is 
    # not listed in the ontology under operation. In this case, sourcing the NamedIndividual to surface that
    # property needs to be performed not from the ontology, but direct from the serialisation specification:
...
def process_serialisation(json_data):
    ...
    #o_staticproperties = [d.iri for d in ontol.data_properties()]
