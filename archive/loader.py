    ...
    for job in batch_manifest.items():
        ...
        for e, row in enumerate(data):
            ...
            for t in row_h_triples:
                if tuple(t) not in triples:
                    triples.append( tuple(t) )

            row_triples = gen_content(onto, row_header, remap(row, serial_key), entities, properties, data_properties, lineage_tree, namespace)
            ...
            for i,individual in enumerate(row_triples):
                if dup_index[i]:
                    print("yes", dup_index[i])
                    print (individual)
                    #reify_triples.extend (reify_triple(onto, row_header, individual)) # keyed on individual triple
                    reify_triples = {**reify_triples, **reify_triple(onto, row_header, individual)} # keyed on individual triple
                else:
                    print("no!", dup_index[i])
        ...
        # At this stage, there's a set of triples where multiple entities share the same unique IDs
        #triples = set(triples)
        pre_load_graph = Graph()
        unique_pointers={}
        for e,(s,p,o) in enumerate(triples):
            if p == URIRef(onto.UniqueIdentifier.iri):
                if o in unique_pointers.keys():
                    unique_pointers[o].append(s)
                else:
                    unique_pointers[o]=[s]

        repointed_masters = {}

        for k,v in unique_pointers.items():
            for i in v:
                repointed_masters[i]=v[0]

        reduced_triples=set()

        for e,(s,p,o) in enumerate(triples):
            repointed_triple = (repointed_masters.get(s,s), repointed_masters.get(p,p), repointed_masters.get(o,o))
            reduced_triples.add(repointed_triple)


        mastered_triples = master_triples(onto, rdflib_graph, reduced_triples)

        triplekeylist=list(reify_triples.keys())
        reified_triple_master_keys = master_triples(onto, rdflib_graph, triplekeylist)
        master_reified_d = {}
        for i,k in enumerate(triplekeylist):
            master_reified_d[reified_triple_master_keys[i]] = master_triples(onto, rdflib_graph, reify_triples[k])

        print("mastered reified counts", len(master_reified_d), len(triplekeylist))
        # All triples mastered for the dataset except the reified triples - these also currently point at the
        # original non-mastered version of the data.

        for k,v in master_reified_d.items():
            if k in mastered_triples:
                mastered_triples.extend(v)
            else:
                print("not here")

        print(len(set(triples)), len(triples), len(reduced_triples), len(mastered_triples))
        if len(set(triples))!=len(triples):
            assert False

        for e,t in enumerate(mastered_triples):
            try:
                if t not in rdflib_graph.triples(t):
                    rdflib_graph.add(t)
                else:
                    print(t, "exists")
...
def reify_triple(onto, collection_node, triple):
    ...
    return {tuple(triple):triples}

def gen_content(onto, row_header_URI, row, entities, properties, data_properties, lineage_tree, namespace):
    triples = []
    top_level_objects = {}
    label_d = {}
    for e in entities:
